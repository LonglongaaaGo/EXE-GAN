...
if __name__ == "__main__":
    ...
    exe_imgs_1 = get_img_lists(args.exemplar_dir, exe1_post)
    ...
    for i in tqdm(range(len(exe_imgs_1))):
        ...
        for jj in range(args.sample_times):
            ...
            for j, g_img in enumerate(completed_img):
                utils.save_image(
                    g_img.add(1).mul(0.5),
                    f"{str(args.eval_dir)}/{name}_{j}_{jj}_inpaint.png",
                    nrow=int(1),
                )

            completed_img = generator.mixing_forward(gt_img_, mask_01, exe_img_2, exe_img_1)

            for j, g_img in enumerate(completed_img):
                utils.save_image(
                    g_img.add(1).mul(0.5),
                    f"{str(args.eval_dir)}/{name}_{j}_{jj}_inpaint2.png",
                    nrow=int(1),
                )

        for j, g_img in enumerate(exe_img_1):
            utils.save_image(
                g_img.add(1).mul(0.5),
                f"{str(args.eval_dir)}/{name}_{j}_exemplar1.png",
                nrow=int(1),
            )
        for j, g_img in enumerate(exe_img_2):
            utils.save_image(
                g_img.add(1).mul(0.5),
                f"{str(args.eval_dir)}/{name}_{j}_exemplar2.png",
                nrow=int(1),
            )